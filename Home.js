...
const Home = ({navigation}) => {
  ...
  //       fetch(
  //         'http://ninecent.shiftlogics.com/api/voucher/viewVoucherAPP',
  //         {
  //           method: 'POST',
  //           headers: new Headers({
  ...
  useEffect(() => {
    ...
    fetch(
      'http://ninecent.shiftlogics.com/api/newpost/viewNewpost',
      {
        method: 'POST',
        headers: new Headers({
          Accept: 'application/json',
        ...
        {item.postImage.map((ee) => {
          return (
            <View style={[{borderRadius: 17}, styles.shadowLayout]}>
              <Image
                source={{uri: `http://ninecent.shiftlogics.com/${ee.pImage}`}}
                style={{
