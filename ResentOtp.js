...
const notificationValues = {
  ...
  const applyRefercode = () => {
    ...
    if (referralCode === '') {
      ...
      fetch(
        'http://ninecent.shiftlogics.com/api/user/referral',
        {
          method: 'POST',
          headers: new Headers({
  ...
  const submitOTPAPI = async () => {
    ...
    if (currentOTP.length == 4) {
      ...
      try {
        dispatch(Ltout(purgeStoredState));
        await dispatch(
          loginPhoneAction(apiToken, currentOTP, phoneNumber,(route?.params.loginfrom == 'Login') ? 'http://ninecent.shiftlogics.com/api/user/loginotpverified' : 'http://ninecent.shiftlogics.com/api/user/otpverified',navigation,notificationValues),
        ).then(() => {
          setIsLoadingList(false);
        });
      } catch {}
  ...
  useEffect(() => {
    if (route?.params.loginfrom == 'Create') {
        ...
        fetch(
          'http://ninecent.shiftlogics.com/api/user/sendotp',
          {
            method: 'POST',
            headers: new Headers({
              Accept: 'application/json',
     ...
     <View style={{marginBottom: 10, marginHorizontal: 20}}>
        ...
        <View
          ...
          {done == false ? (
            ...
            <TouchableOpacity onPress={() => resendOTPAPI((route?.params.loginfrom == 'Login') ? 'http://ninecent.shiftlogics.com/api/user/loginphone' : 'http://ninecent.shiftlogics.com/api/user/sendotp')}>
              <View
                style={[
                  {
                    padding: 10,
